...
const CommandTerminal: React.FC<CommandTerminalProps> = ({ 
  ...
  // Sync local input state with config prop from parent (which comes from localStorage)
  const [localUwKey, setLocalUwKey] = useState(config.uwApiKey);
  const [localDiscord, setLocalDiscord] = useState(config.discordWebhook);

  // Update local state if the parent config changes (e.g., initial load from localStorage)
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full gap-6 font-mono">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1 min-h-0">
        ...
        {/* Left: Input Area (Data submission) */}
        <div className="lg:col-span-2 flex flex-col bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl relative">
          ...
          <textarea
            ...
            className="flex-1 bg-transparent p-6 text-emerald-500/80 text-xs focus:outline-none resize-none custom-scrollbar leading-relaxed"
            placeholder="粘贴 AnalysisPacket JSON 数据..."
          />
          <div className="p-4 bg-zinc-900/20 border-t border-zinc-900 flex gap-4">
            <button 
              onClick={handleDeploy}
              className="px-6 py-2 bg-emerald-600/10 text-emerald-500 border border-emerald-500/20 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] hover:bg-emerald-600 hover:text-white transition-all"
            >
              部署到监控屏
            </button>
            <button 
              ...
              className={`px-6 py-2 bg-blue-600/10 text-blue-500 border border-blue-500/20 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] hover:bg-blue-600 hover:text-white transition-all ${isProcessing ? 'opacity-50 cursor-wait' : ''}`}
            >
        ...
        {/* Middle: External Integration Config */}
        <div className="flex flex-col bg-zinc-950 border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl">
          <div className="bg-zinc-900/50 px-4 py-2 border-b border-zinc-800 flex justify-between items-center">
            ...
            <span className="text-[7px] font-black text-emerald-500/50 uppercase tracking-widest flex items-center gap-1">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <input 
                  ...
                  className="w-full bg-black/40 border border-zinc-900 rounded-lg px-3 py-2 text-[10px] text-zinc-400 focus:outline-none focus:border-emerald-500/30 transition-all font-mono"
                  placeholder="请输入 API Key..."
                />
              </div>
              <p className="text-[7px] text-zinc-700 leading-tight">注：用于实时同步 0DTE / 1DTE GEX 原始流量包。更改即刻生效。</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <input 
                  ...
                  className="w-full bg-black/40 border border-zinc-900 rounded-lg px-3 py-2 text-[10px] text-zinc-400 focus:outline-none focus:border-blue-500/30 transition-all font-mono"
                  placeholder="https://discord.com/api/webhooks/..."
                />
              </div>
              <p className="text-[7px] text-zinc-700 leading-tight">注：信号触发时自动推送到指定频道。修改自动存档。</p>
            </div>
            
            <div className="p-3 bg-indigo-500/5 border border-indigo-500/10 rounded-xl">
               <div className="flex items-center gap-2 mb-1">
                 <i className="fa-solid fa-shield-halved text-indigo-400 text-[8px]"></i>
                 <span className="text-[8px] font-black text-indigo-400 uppercase tracking-widest">安全建议</span>
               </div>
               <p className="text-[8px] text-zinc-600 leading-relaxed italic">"密钥仅存储于本地浏览器缓存，不会上传到哨兵主服务器。"</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col bg-black border border-zinc-900 rounded-2xl overflow-hidden shadow-2xl">
          ...
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar text-[10px] space-y-1">
            {logs.map((log, i) => (
          ...
          <div className="p-3 bg-zinc-950 border-t border-zinc-900 flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[8px] text-zinc-600 font-bold uppercase">链路活跃</span>
            </div>
            <button 
              onClick={() => setLogs([])}
              className="text-[8px] text-zinc-700 hover:text-zinc-500 uppercase font-black"
            >
              清除日志
            </button>
          </div>
      ...
      <div className="h-20 bg-zinc-950/40 border border-zinc-900/50 rounded-2xl p-4 flex items-center justify-around">
        <div className="flex flex-col items-center">
          <span className="text-[8px] text-zinc-600 font-black uppercase mb-1">UnusualWhales 状态</span>
          <span className={`text-xs font-bold tracking-tighter ${config.uwApiKey ? 'text-emerald-500' : 'text-zinc-600'}`}>
            {config.uwApiKey ? '认证激活' : '等待输入'}
          </span>
        </div>
        <div className="h-8 w-px bg-zinc-900"></div>
        <div className="flex flex-col items-center">
          <span className="text-[8px] text-zinc-600 font-black uppercase mb-1">上行 Discord 链路</span>
          <span className={`text-xs font-bold tracking-tighter ${config.discordWebhook ? 'text-blue-500' : 'text-zinc-600'}`}>
            {config.discordWebhook ? '链路已就绪' : '离线状态'}
          </span>
        </div>
        <div className="h-8 w-px bg-zinc-900"></div>
        <div className="flex flex-col items-center">
          <span className="text-[8px] text-zinc-600 font-black uppercase mb-1">加密套件</span>
          <span className="text-xs text-zinc-400 font-bold tracking-tighter text-emerald-500/70">AES-256-LOCAL</span>
        </div>
      </div>
    </div>
  );
